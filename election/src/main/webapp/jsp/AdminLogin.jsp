...
try
{
	...
	if(request.getParameter("btn_login")!=null) //check login button click event not null
	{
		...
		if(rs.next())
		{
			...
			if(email.equals("admin@test.com") && password.equals("111111111"))
			{
				session.setAttribute("login","admin@test.com"); //session name is login and store fetchable database email address
				response.sendRedirect("welcomeAdmin.jsp"); //after login success redirect to welcome.jsp page
			}
		}
		else
		{
			request.setAttribute("errorMsg","Kindly Login with the Admin Information"); //invalid error message for email or password wrong
		}
